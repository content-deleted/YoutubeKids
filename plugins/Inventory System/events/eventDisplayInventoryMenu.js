export const id = "INVENTORY_DISPLAY_MENU";
export const name = "Display Inventory Menu";
...
appendRaw(`VM_SET_CONST_UINT8 _show_actors_on_overlay, ${input.showActors ? 1 : 0}`);
  ...
  _overlayClear(boxX-1, boxY-1, menuWidth, menuHeight, ".UI_COLOR_WHITE", input.showBorder
      );
  

  _overlayMoveTo(0, 0, speedIn);
  
  _callNative("inventory_get_item_names");
  _displayText();
  ...
  Array(total)
    ...
    .map((_, i) => {
      ...
      const idx = i + 1;
      const fieldX = 0;// input[`option${idx}_x`] || 0;
      const fieldY = i+1;//input[`option${idx}_y`] || 0;
      const left = idx;//clampItem(input[`option${idx}_l`]);
      const right = idx;//clampItem(input[`option${idx}_r`]);
      let up = idx - 1;//clampItem(input[`option${idx}_u`]);
      let down = idx + 1;//clampItem(input[`option${idx}_d`]);
      if(down > total) down = 1;
      if(up < 0) up = total;
